# Properties of Fast Radio Bursts {#sec-properties}
...
The redshift of FRB ...
<!-- TODO: Describe IGM to calculate IGM, @hashimoto_EnergyFunctionsFast_2022, @hashimoto_NoRedshiftEvolution_2020, @hashimoto_LuminosityDurationRelation_2019 -->
...
\begin{align}
E_{\nu_\text{rest}} 
&= \int_\text{pulse} \frac{4\pi D_L(z)^2}{(1+z)^{1+\alpha}}\left(\frac{\nu_\text{rest}}{\nu_\text{obs}}\right)^\alpha S_{\nu_\text{obs}} \text{d}t, \\
&= \frac{4\pi D_L(z)^2}{(1+z)^{2+\alpha}}\left(\frac{\nu_\text{rest}}{\nu_\text{obs}}\right)^\alpha\mathcal{F}_{\nu_\text{obs}},\\
& \text{(in ergs Hz$^{-1}$)}
\end{align}
$$ {#eq-energy-by-time-integrated-luminosity}
where $D_L(z)$ is the luminosity distance calculated from redshift and -- following @eq-fluence-by-integral -- $S_\nu_\text{obs}$ is the flux density at the observed frequency and $\mathcal{F}_\nu_\text{obs}$ is its fluence.
