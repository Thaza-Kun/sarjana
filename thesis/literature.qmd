...
As a consequence, its arrival time is directly dependent on its frequency with lower frequencies arriving later (Day, 2022; Kulkarni, 2020).
The measure of this dispersion – its dispersion measure (DM) – is a key observable quantity in studies of FRBs (Keane et al., 2016).
...
For the sake of precision, both values with its uncertainty are provided here (Kulkarni, 2020):
...
It then is characterized by the expression (Petroff et al., 2019)
$$
...
This quantity, RM, is analogous to @eq-DM for DM as it is the total interaction along the line of sight (Brentjens & de Bruyn, 2005; Feng et al., 2022),
$$
...
The YMW16 model (Yao et al., 2017) uses known distances of pulsars using independent methods and matching it with their respective DMs.
Its value highly depends on its galactic latitude as there is little material higher up in the latitude so its DM is expected to be small (Thornton et al., 2013).
Non-Milky Way contributions to the observed DM (dubbed dispersion measure excess, $\text{DM}_\text{E}$) come from the intergalactic medium (IGM) and the host galaxy (Deng & Zhang, 2014; Petroff et al., 2019),
$$
...
The $\text{DM}_\text{Host}$ of the transient depends its distance from the host galactic core and viewing angle (Thornton et al., 2013) because those factor determines how much material interacts with the signal throughout its propagation.
