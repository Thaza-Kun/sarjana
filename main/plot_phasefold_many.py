...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--selects", action="extend", nargs="+", type=str, required=True
    )
    parser.add_argument("--runs", type=int, default=100)
    parser.add_argument(
        "--periods", type=float, action="extend", nargs="+", required=True
    )
...
assert_same_length = "\n".join(
    [
        "Arguments for `--names` and `--periods` are not the same length. Current values:",
        ", ".join(
...
fig, axes = plt.subplots(
    NROW, NCOL, figsize=(FIGSCALE * 8.72, FIGSCALE * 8.30), sharex=True, sharey=True
)
...
for i, (name, period) in enumerate(zip(names, fold_periods)):
    ...
    exp_curve, exp_bins = phasefold(
        exposure_date, period, values=exposures_hr, shift=exposure_date.min()
    )
    ...
    active = np.array(
        list(
            map(
                lambda x: (x[0], x[-1] + 1 / NBINS, x[-1] - x[0] + 1 / NBINS),
    ...
    for s in spans:
        if not s.active:
            if headtail > s.length and not (
                s.end == tailspan.end or s.end == headspan.end
            ):
                continue
