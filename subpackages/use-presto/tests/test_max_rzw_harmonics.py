...
for n in range(numtrials):
    ...
    if numharm > 1:
        [maxpow, rmax, zmax, rds] = presto.maximize_rz_harmonics(
            ft, r + offset[0], z + offset[1], numharm, norm=1.0
        )
    else:
        [maxpow, rmax, zmax, rd] = presto.maximize_rz(
            ft, r + offset[0], z + offset[1], norm=1.0
        )
    rztime += clk() - a
    ...
    a = clk()
    if numharm > 1:
        [maxpow, rmax, zmax, wmax, rds] = presto.maximize_rzw_harmonics(
            ft, r + offset[0], z + offset[1], w + offset[2], numharm, norm=1.0
        )
    else:
        [maxpow, rmax, zmax, wmax, rd] = presto.maximize_rzw(
            ft, r + offset[0], z + offset[1], w + offset[2], norm=1.0
        )
    rzwtime += clk() - a
    rzwerrs[n] = (
        (maxpow / numharm - theo_max_pow) / theo_max_pow,
        rmax - r,
        zmax - z,
        wmax - w,
    )
