...
fig, axs = plt.subplots(2, 2, figsize=(9, 8), gridspec_kw={'hspace': 0, 
                                                        'wspace': 0,
                                                        'width_ratios': [5, 1],
                                                        'height_ratios': [1, 5]})
axs[0,0].axis("off")
...
axs[0,0].plot(histcollection[1][point5_peak2], [0.01]*len(histcollection[1][point5_peak2]), '|', color=[*colors.values()][3])
axs[0,0].plot(histcollection[0][point5_peak], [0.01]*len(histcollection[0][point5_peak]), '|', color=[*colors.values()][2])
axs[1, 1].hist(
    ihistcollection,
...
axs[1,1].plot([0.5]*len(ihistcollection[1][point5_peak2]),ihistcollection[1][point5_peak2],  '|', color=[*colors.values()][3])
axs[1,1].plot([0.5]*len(ihistcollection[0][point5_peak]),ihistcollection[0][point5_peak],  '|', color=[*colors.values()][2])

plt.suptitle(name)
axs[1, 0].set(xlabel=r"$\chi^2$ / ($\Phi$ - 1)", ylabel=r"$F_0$")
axs[1, 0].legend(loc='lower right')
plt.tight_layout()
plt.savefig(pathlib.Path(outdir, f"Composite-periodogram-{name}.png"))
