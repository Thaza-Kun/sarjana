...
def main(arguments: argparse.Namespace):
    ...
    if (len(sim_ensemble.power) != 0) and (len(frb_ensemble.power) != 0):
        ...
        if arguments.rate:
            plt.suptitle(
                f"{name} | events={len(thisfrb.signal)} | burst rate = {arguments.rate:.2e} 1/h"
            )
        else:
            plt.suptitle(f"{name} | events={len(thisfrb.signal)}")
        plt.tight_layout()
        plt.savefig(pathlib.Path(simdir, f"n{runs:0>6}-min_SNR={cutoff_SNR}.png"))
