...
class FRBPopulation:
    ...
    async def download_csv(self: object, url:str='https://www.herta-experiment.org/frbstats/catalogue.csv', to_file :str='population.csv') -> str:
        result = await request_data(url)
        output_file: Path = Path.joinpath(DATAPATH, to_file)
