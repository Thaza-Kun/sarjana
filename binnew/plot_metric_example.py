...
for i, (s, b) in enumerate(itertools.product(shifts, NBINS)):
    ...
    exp_curve, exp_bins = phasefold(exposure_date, predict_period, values=exposures_hr, shift=exposure_date.min()+s, nbins=b)
    ...
    active = np.array(list(map(lambda x: (x[0], x[-1] + 1/b, x[-1]-x[0] + 1/b), split_by_mask(bins[:-1], active_phase))))
    ...
    ax0.step(exp_bins, [*exp_curve, 0], color="tab:green", alpha=0.3, linestyle="--", where="post", label=r"$r \cdot E_\phi$")
    ...
    for s in spans:
        if not s.active:
            ...
            ax0.text(s.start + 0.5*(s.end - s.start), textheight, r'$F_0$ = ' + f'{span_length:.2f}', ha='center', va='center', size=11)
            break
    ...
    ax1.text(1, 1, r'$\chi^2 / (\Phi - 1)$ = ' + f'{chisquare(curve, exp_curve).statistic / (b - 1):.2f}', ha='right', va='top', size=11, transform=ax1.transAxes)
...
fig.savefig(pathlib.Path(outdir, f"Example-phasefold-metric-{name}~{predict_period:.2f}x{'+'.join(str(b) for b in NBINS)}x{'+'.join(str(s) for s in shifts)}.pdf"))
print(pathlib.Path(outdir, f"Example-phasefold-metric-{name}~{predict_period:.2f}x{'+'.join(str(b) for b in NBINS)}x{'+'.join(str(s) for s in shifts)}.pdf"))
plt.close()
