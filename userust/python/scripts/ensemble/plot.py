...
def main(arguments: argparse.Namespace):
    ...
    if arguments.period:
        line = axs[1, 0].axvline(arguments.period, color="k", linestyle=":", alpha=0.5)
        axs[1, 0].annotate(
            f"{arguments.period:.2f}", (0, 0.95), xycoords=line, ha="left", va="top"
        )
    ...
    plt.suptitle(
        f"{PDGRAM[pdgram_name]} Periodogram Profile for {name}", fontsize="x-large"
    )
    plt.tight_layout()
    plt.savefig(
        pathlib.Path(
